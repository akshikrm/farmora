...
import 'package:farmora/screens/general_expenses/add_general_expense.dart';
...
class GeneralExpensesListingPage extends StatefulWidget {
  const GeneralExpensesListingPage({super.key});

  @override
  State<GeneralExpensesListingPage> createState() =>
      _GeneralExpensesListingPageState();
}

class _GeneralExpensesListingPageState
    extends State<GeneralExpensesListingPage> {
  int? _selectedSeason;
  DateTime? _startDate;
  DateTime? _endDate;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Initially clear or fetch default? Let's prompt user to filter like sales book
      context.read<GeneralExpensesProvider>().clearExpenses();
    });
  }
  ...
  void _applyFilters() {
    context.read<GeneralExpensesProvider>().fetchExpenses(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          NavigationUtils.navigateTo(context, const AddGeneralExpensePage())
              .then((value) {
            if (value == true) {
              _applyFilters();
            }
      ...
      body: Column(
        children: [
          _buildFilterSection(),
          Expanded(
            child: Consumer<GeneralExpensesProvider>(
              builder: (context, provider, child) {
                if (provider.isLoading) {
                  return const Center(child: CircularProgressIndicator());
                }

                if (provider.expensesList.isEmpty) {
                ...
                for (var item in provider.expensesList) {
                  totalExpense +=
                ...
                return ListView(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    ...provider.expensesList.map((item) {
                      return Padding(
